...
{
    if( !m_isLoadlib )//静态变量，库只能加载一次，防止多次加载
    {
        //1.做个小买卖 炸串 --加载库
...
*/
    ...
    int DataLen = nlen + 4;//数据总的长度为接收的数据的长度nlen+4  4是为了标记包的大小
    std::vector<char> vecbuf;//申请一个动态数组
    vecbuf.resize( DataLen );//相当于初始化该动态数组的容量

 //   char* buf = new char[ DataLen ];
    char* buf = &*vecbuf.begin();//该动态数组的首元素赋给buf，begin不就是类似一个迭代器指针么
    char* tmp = buf;//再把buf给tmp
    *(int*) tmp = nlen;//初始化tmp该值
    tmp+= sizeof(int);//加上一个int的长度，此时的tmp被强转为了int

    memcpy( tmp , szbuf , nlen);//将szbuf里的内容赋给tmp->buf

    int res = send( lSendIP ,buf,DataLen , 0);//f
//    delete[] buf;
    return res;
}
//接收
