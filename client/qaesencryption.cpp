...
//最终的调用接口
//参数pswBytesSize即 AES_128,AES_192,AES_256
QString QAESEncryption::encodeAesOnECB(QString originText, QString passWord, Aes pswBytesSize)
{
    //参数1,密码长度,对passWord长度无要求,如与参数不符，本类将自动处理
    //AES_128　密码长128位16字节
    //AES_192　密码长128位24字节
    //AES_256　密码长128位32字节
    //参数2,工作模式:ECB独立块模式、CBC块链模式
    //参数3,填充模式(QAESEncryption::ZERO可确保填充后加明文密文仍相同，不会多出空格回车之类)
    QAESEncryption encryption(pswBytesSize, QAESEncryption::ECB,QAESEncryption::ZERO);
    //保证密码字节长度与pswBytesSize一致
    QByteArray pswBytes;
    switch (pswBytesSize) {
    case QAESEncryption::AES_128:
        pswBytes=QCryptographicHash::hash(passWord.toLocal8Bit(),QCryptographicHash::Md5);
        break;
    case QAESEncryption::AES_192:
        pswBytes=hash192(passWord.toLocal8Bit());
        break;
    case QAESEncryption::AES_256:
        pswBytes=QCryptographicHash::hash(passWord.toLocal8Bit(),QCryptographicHash::Sha256);
        break;
    }
    QByteArray encodedBytes= encryption.encode(originText.toLocal8Bit(), pswBytes);
    return QString::fromLatin1(encodedBytes.toBase64());
}

QString QAESEncryption::decodeAesOnECB(QString encodeText, QString passWord,Aes pswBytesSize)
{
    QAESEncryption encryption(pswBytesSize, QAESEncryption::ECB,QAESEncryption::ZERO);
    //保证密码字节长度与pswBytesSize一致
    QByteArray pswBytes;
    ...
    QByteArray decodedBytes=encryption.decode(QByteArray::fromBase64(encodeText.toLatin1()), pswBytes);
    return QString::fromLocal8Bit(decodedBytes);
}

QString QAESEncryption::encodeAesOnCBC(QString originText, QString passWord, QString ivStr, QAESEncryption::Aes pswBytesSize)
{
    //CBC工作模式对密码长度及偏移向量串长度要求严格
    //参数1,密码长度,对passWord长度无要求,如与参数不符，本类将自动处理
    //AES_128　密码长128位16字节
    //AES_192　密码长128位24字节
    //AES_256　密码长128位32字节
    //参数2,工作模式:ECB独立块模式、CBC块链模式
    //参数3,填充模式(QAESEncryption::ZERO可确保填充后加明文密文仍相同，不会多出空格回车之类)
    QAESEncryption encryption(pswBytesSize, QAESEncryption::CBC,QAESEncryption::ZERO);
    //保证密码字节长度与pswBytesSize一致
    QByteArray pswBytes;
    switch (pswBytesSize) {
    case QAESEncryption::AES_128:
    ...
    //密码偏移向量,必须为16个字节
    QByteArray ivBytes = QCryptographicHash::hash(ivStr.toLocal8Bit(), QCryptographicHash::Md5);
    QByteArray encodeBytes = encryption.encode(originText.toLocal8Bit(), pswBytes, ivBytes);

    return QString::fromLatin1(encodeBytes.toBase64());
}

QString QAESEncryption::decodeAesOnCBC(QString encodeText, QString passWord, QString ivStr, QAESEncryption::Aes pswBytesSize)
{
    //CBC工作模式对密码长度及偏移向量串长度要求严格
    //参数1,密码长度,对passWord长度无要求,如与参数不符，本类将自动处理
    //AES_128　密码长128位16字节
    ...
    //密码偏移向量,必须为16个字节
    QByteArray ivBytes = QCryptographicHash::hash(ivStr.toLocal8Bit(), QCryptographicHash::Md5);
    QByteArray decodeBytes = encryption.decode(QByteArray::fromBase64(encodeText.toLatin1()), pswBytes, ivBytes);

    return QString::fromLocal8Bit(decodeBytes);
}

QByteArray QAESEncryption::hash192(QByteArray originBytes)
{
    QByteArray newBytes=QCryptographicHash::hash(originBytes,QCryptographicHash::Sha384);
    QByteArray resultBytes;
    for (int i = 0; i < 24; i++) {
        char char1=newBytes.at(i*2);
        char char2=newBytes.at(i*2+1);
        char char3=(char1+char2)/2;
        resultBytes.append(char3);
    }   
    return resultBytes;
}
